...
# def get_all_groups_recursive(conn, starting_group_dns, visited=None):
#     """
#     DFS/BFS up the chain using memberOf to gather all parent groups, including nested.
#     """
#     if visited is None:
#         visited = set()
#     stack = list(starting_group_dns)
#     result = set()
...
#         # Read only this group (BASE scope) to fetch its parents via memberOf
#         ok = conn.search(
#             search_base=dn,
#             search_scope=BASE,
...
#             parents = entry.memberOf.values if hasattr(entry, 'memberOf') else []
#             for p in parents:
#                 if p not in visited:
#                     stack.append(p)
...
# ---- Drop-in usage right after direct groups are fetched ----
# You already have direct groups:
...
def login():
    ...
    if request.method == 'POST':
        ...
        service_conn.search(
            search_base="DC=vistancenetworks,DC=com",
            search_filter="(&(objectClass=group)(cn=#T*))",
    ...
    #         ADMIN_USERS = ["shravani.mandala@ruckusnetworks.com"]
